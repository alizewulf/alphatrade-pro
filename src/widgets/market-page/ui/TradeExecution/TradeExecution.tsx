...
function TradeExecution() {
  ...
  return (
    <section className="flex flex-col gap-6 w-3/12 outline outline-[#424754]/10 bg-[#1E293B]/70 rounded-sm">
      <TradeExecutionHeader state={operationType} setState={setOperationType} />
      ...
      <div className="mt-3 flex px-6 flex-col gap-2">
        <div className="w-full flex justify-between text-sm text-paragraph leading-5">
          <p>{operationType === "buy"? "Buy" : "Sell"} Power</p>
        ...
        <div className="flex w-full my-4 px-6 rounded-lg justify-center">
          <button
            className={`w-full capitalize py-4 ${operationType === "buy" ? "bg-secondary text-[#003824]" : "text-negative outline outline-negative bg-negative/15"}  font-semibold text-xl leading-7 ${buttonDisable ? "" : "cursor-pointer"} rounded-lg`}
            disabled={buttonDisable}
            onClick={() => trade.setOperationType(operationType)}
          >
            {operationType}
          </button>
        </div>
